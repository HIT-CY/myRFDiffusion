...
class SignalDiffusion(nn.Module):
    def __init__(self, params):
        ...
        self.input_dim = self.params.sample_rate  # input time-series data length, N , 512
        self.extra_dim = self.params.extra_dim  # dimension of each data sample, e.g., [S A 2] for complex-valued CSI
        self.max_step = self.params.max_step  # maximum diffusion steps, 100
    ...
    def get_kernel(self, var_kernel):
        ...
        gaussian_kernel = torch.exp(-((samples - self.input_dim // 2) ** 2) / (2 * var_kernel)) / torch.sqrt(
            2 * torch.pi * var_kernel)  # G_t, [T, N]
        gaussian_kernel = self.input_dim * gaussian_kernel / torch.sum(gaussian_kernel, dim=1,
                                                                       keepdim=True)  # Normalized G_t, [T, N]
    ...
    def get_noise_weights(self):
        ...
        for t in range(self.max_step):
            upper_bound = t + 1
            one_minus_alpha_sqrt = torch.sqrt(
                1 - self.alpha[0:upper_bound])  # \sqrt(1-\bar{\alpha_s}), for s in [1, t], [t]
            rev_one_minus_alpha_sqrt = torch.flipud(
                one_minus_alpha_sqrt)  # \sqrt(1-\bar{\alpha_s}), for s in [t, 1], [t]
            rev_alpha = torch.flipud(self.alpha[0:upper_bound])  # alpha_s, for s in [t, 1], [t]
            rev_alpha_bar_sqrt = torch.sqrt(torch.cumprod(rev_alpha, dim=0) / rev_alpha[
                -1])  # \sqrt{\bar{\alpha_t} / \bar{\alpha_s}}, for s in [t, 1], [t]
            ...
            noise_weights.append(torch.mv((rev_alpha_bar_sqrt.unsqueeze(-1) * rev_kernel_bar).transpose(0, 1),
    ...
    def get_noise_weights_stats(self):
        ...
        for t in range(self.max_step):
            noise_weights.append((1 - torch.sqrt(self.alpha_bar[t]) * self.gaussian_kernel_bar[t, :]) / (
                        1 - torch.sqrt(self.alpha[0]) * self.gaussian_kernel[0, :]))
    ...
    def get_noise_weights_div(self):
        ...
        for t in range(self.max_step):
            upper_bound = t + 1
            one_minus_alpha_sqrt = torch.sqrt(
                1 - self.alpha[:upper_bound])  # \sqrt(1-\bar{\alpha_s}), for s in [1, t], [t]
            ratio_alpha_bar_sqrt = torch.sqrt(self.alpha_bar[t] / self.alpha_bar[
                                                                  :upper_bound])  # \sqrt(\bar{\alpha_t} / \bar{\alpha_s}), for s in [1, t], [t]
            ratio_kernel_bar = self.gaussian_kernel_bar[t, :] / self.gaussian_kernel_bar[:upper_bound,
                                                                :]  # \bar{G_t} / \bar{G_s}, for s in [1, t], [t, N]
            noise_weights.append(torch.mv((ratio_alpha_bar_sqrt.unsqueeze(-1) * ratio_kernel_bar).transpose(0, 1),
                                          one_minus_alpha_sqrt))  # [t, N]
        return torch.stack(noise_weights, dim=0)  # [T, N]
    ...
    def get_noise_weights_prod(self):
        ...
        for t in range(self.max_step):
            ...
            rev_kernel_bar = torch.cumprod(rev_kernel, dim=0) / rev_kernel[-1,
                                                                :]  # \bar{G_t} / \bar{G_s}, for s in [t, 1], [t, N]
            noise_weights.append(torch.mv((rev_alpha_bar_sqrt.unsqueeze(-1) * rev_kernel_bar).transpose(0, 1),
                                          rev_one_minus_alpha_sqrt))  # [t, N]
        return torch.stack(noise_weights, dim=0)  # [T, N]
    ...
    def degrade_fn(self, x_0, t, task_id=0):
        ...
        if task_id in [0, 1]:
            noise_weight = self.noise_weights[t, :].unsqueeze(-1).unsqueeze(-1).to(
                device)  # equivalent gaussian noise weights, [B, N, 1, 1, 1]
            info_weight = self.info_weights[t, :].unsqueeze(-1).unsqueeze(-1).to(
                device)  # equivalent original info weights, [B, N, 1, 1, 1]
        if task_id in [2, 3]:
            noise_weight = self.noise_weights[t, :].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to(
                device)  # equivalent gaussian noise weights, [B, N, 1, 1, 1]
            info_weight = self.info_weights[t, :].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to(
                device)  # equivalent original info weights, [B, N, 1, 1, 1]
    ...
    def sampling(self, restore_fn, cond, device):
        ...
        if self.task_id in [2, 3]:
            inf_weight = (self.noise_weights[batch_max, :] + self.info_weights[batch_max, :]).unsqueeze(-1).unsqueeze(
                -1).unsqueeze(-1).to(device)  # [B, N, 1, 1, 1]
        else:
            inf_weight = (self.noise_weights[batch_max, :] + self.info_weights[batch_max, :]).unsqueeze(-1).unsqueeze(
                -1).to(device)  # [B, N, 1, 1, 1]
        x_s = inf_weight * noise  # [B, N, S, A, 2]
        # Restore data from noise.
        for s in range(self.max_step - 1, -1, -1):  # reverse from t to 0
            x_0_hat = restore_fn(x_s, s * torch.ones(batch_size, dtype=torch.int64),
                                 cond)  # resotre \hat{x_0} from x_s using trained tfdiff model
            if s > 0:
                # x_{s-1} = D(\hat{x_0}, s-1)
                x_s = self.degrade_fn(x_0_hat, t=(s - 1) * torch.ones(batch_size, dtype=torch.int64),
                                      task_id=self.task_id)  # degrade \hat{x_0} to x_{s-1}
        return x_0_hat
        ...
        for s in range(self.max_step - 1, -1, -1):  # reverse from t to 0
            ...
            if s > 0:
                ...
                x_s = x_s - self.degrade_fn(x_0_hat, t=s * torch.ones(batch_size, dtype=torch.int64),
                                            task_id=self.task_id) + self.degrade_fn(x_0_hat,
                                                                                    t=(s - 1) * torch.ones(batch_size,
                                                                                                           dtype=torch.int64),
                                                                                    task_id=self.task_id)  # degrade \hat{x_0} to x_{s-1}
        return x_0_hat
...
class GaussianDiffusion(nn.Module):
    ...
    def degrade_fn(self, x_0, t):
        device = x_0.device
        noise_weight = self.noise_weights[t].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to(
            device)  # equivalent gaussian noise weights, [B, 1, 1, 1]
        info_weight = self.info_weights[t].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to(
            device)  # equivalent original info weights, [B, 1, 1, 1]
    ...
    def sampling(self, restore_fn, cond, device):
        ...
        for s in range(self.max_step - 1, -1, -1):  # reverse from t to 0
            ...
            if s > 0:
                # x_{s-1} = D(\hat{x_0}, s-1)
                x_s = self.degrade_fn(x_0_hat, t=(s - 1) * torch.ones(batch_size,
                                                                      dtype=torch.int64))  # degrade \hat{x_0} to x_{s-1}
        return x_0_hat
    ...
    def robust_sampling(self, restore_fn, cond, device):
        ...
        for s in range(self.max_step - 1, -1, -1):  # reverse from t to 0
            ...
            if s > 0:
                # x_{s-1} = x_s - D(\hat{x_0}, s) + D(\hat{x_0}, s-1)
                x_s = x_s - self.degrade_fn(x_0_hat, t=[s]) + self.degrade_fn(self, x_0_hat,
                                                                              t=(s - 1) * torch.ones(batch_size,
                                                                                                     dtype=torch.int64))  # degrade \hat{x_0} to x_{s-1}
        return x_0_hat
